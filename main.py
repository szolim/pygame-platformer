...
def main():
    ...
    setup_lib.set_settings(os.path.join(os.path.dirname(os.path.realpath(__file__)),"settings.json"))
    ...
    #Creating a level
    level = levels_lib.Level(level_size=(640,64))
    ...
    while running:
        ...
        #Calculating physics
        player.update(level.tile_rect_map, (level.rows*tile_size, level.columns*tile_size))
        camera.update(player, game_canvas.get_size(), (level.rows, level.columns), tile_size)
